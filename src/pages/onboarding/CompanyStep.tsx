  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h2 className="font-display font-bold text-2xl text-text-bright">Here's what we found</h2>
        <p className="text-sm text-text-secondary mt-2 max-w-xl">
          Edit anything that doesn't look right. This shapes the prompts and competitors we'll use next.
        </p>
      </div>
      ...
      <div className="card-elevated space-y-5">
            ...
            {company.differentiators.map((d, i) => (
              <div key={i} className="flex items-center gap-2">
                ...
                <input
                  value={d}
                  onChange={(e) => {
                    const next = [...company.differentiators];
                ...
                <button
                  onClick={() =>
                    update({
                      differentiators: company.differentiators.filter((_, j) => j !== i),
                    })
                  }
              ...
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === 'Enter' && newDiff.trim()) {
                    update({ differentiators: [...company.differentiators, newDiff.trim()] });
                    setNewDiff('');
                  }
                }}
              />
        ...
        <Field label="Identified direct competitors">
            ...
            {company.competitors.map((c) => (
              ...
              >
                ...
                <button
                  onClick={() =>
                    update({
                      competitors: company.competitors.filter((x) => x.id !== c.id),
                    })
                  }
                ...
                onKeyDown={(e) => {
                  if (e.key === 'Enter' && newComp.trim()) {
                    update({
                      competitors: [
                        ...company.competitors,
                        { id: 'c' + Date.now(), name: newComp.trim(), domain: newComp.trim().toLowerCase().replace(/\s+/g, '') + '.com' },
                      ],
                    });
                    setNewComp('');
                  }
